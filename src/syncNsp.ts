...
syncNsp.addEventHandler(
  new SyncEventHandler(
    "event:join-room",
    (socket: SyncSocket, { room }, cb) => {
      socket.join(room as string);
      if (cb && room) cb("joined room demo-party");
    },
  ),
);
...
syncNsp.addEventHandler(
  new SyncEventHandler(
    "event:play",
    (socket: SyncSocket, { detail, toRoom }, cb) => {
      socket.broadcast
        .to(toRoom)
        .volatile.emit("event:play", { detail, from: socket.id });
      if (cb) cb(`sent to ${toRoom} event play`);
    },
  ),
);
...
syncNsp.addEventHandler(
  new SyncEventHandler(
    ...
    (socket: SyncSocket, { detail, toRoom }, cb) => {
      ...
      if (cb) cb(`sent to ${toRoom} event pause`);
    },
  ),
);
...
syncNsp.addEventHandler(
  new SyncEventHandler(
    ...
    (socket: SyncSocket, { detail, toRoom }, cb) => {
      ...
      if (cb) cb(`sent to ${toRoom} event play-fail`);
    },
  ),
);
...
syncNsp.addEventHandler(
  new SyncEventHandler(
    ...
    (socket: SyncSocket, { detail, toRoom }, cb) => {
      ...
      if (cb) cb(`sent to ${toRoom} event seek`);
    },
  ),
);
...
syncNsp.addEventHandler(
  new SyncEventHandler(
    ...
    (socket: SyncSocket, { detail, toRoom }, cb) => {
      ...
      if (cb) cb(`sent to ${toRoom} event waiting`);
    },
  ),
);
...
syncNsp.addEventHandler(
  new SyncEventHandler(
    ...
    (socket: SyncSocket, { detail, toRoom }, cb) => {
      ...
      if (cb) cb(`sent to ${toRoom} event rate-cahnge`);
    },
  ),
);
