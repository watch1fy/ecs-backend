...
chatNps.addEventHandler(new EventHandler('join-room', (socket: Socket, room, cb) => {
  socket.join(room);
  cb('joined room demo-party')
}))

chatNps.addEventHandler(new EventHandler('message', (socket: Socket, { detail, toRoom }, cb) => {
  socket
    .broadcast
    .to(toRoom)
