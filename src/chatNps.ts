...
chatNps.addEventHandler(
  new ChatEventHandler(
    "event:join-room",
    (socket: ChatSocket, { room }: ChatInPayload, cb) => {
...
chatNps.addEventHandler(
  new ChatEventHandler(
    "event:message",
    (socket: ChatSocket, { message, toRoom }: ChatInPayload, cb) => {
      socket.broadcast
        .to(toRoom)
        .emit("event:message", { message, from: socket.id });
      if (cb) cb(`sent to message ${toRoom}`);
    },
  ),
);
